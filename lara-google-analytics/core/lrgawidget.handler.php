	...
	if ($lrdata['action'] === "lrgawidget_hideShowWidget"){
		...
		switch ($lrdata['action']) {
			...
			case "lrgawidget_getAccessToken":
				...
				break;	
			case "lrgawidget_getProfiles":
				if (in_array("admin", DataStore::$RUNTIME["permissions"])){$call->getProfiles();}
				else{ ErrorHandler::FatalError(__('You do not have permission to access this tab!', 'lara-google-analytics'));}		
				break;
			case "lrgawidget_setProfileID":
				if (in_array("admin", DataStore::$RUNTIME["permissions"])){ $call->setProfileID($lrdata);}
				else{ ErrorHandler::FatalError(__('You do not have permission to access this tab!', 'lara-google-analytics'));}			
				break;
			case "lrgawidget_settingsReset":
				...
				break;			
			case "lrgawidget_getSessions":
				if (in_array("sessions", DataStore::$RUNTIME["permissions"])){$call->getSessions();}
				else{ ErrorHandler::FatalError(__('You do not have permission to access this tab!', 'lara-google-analytics'));}
				break;
